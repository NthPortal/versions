 ...
 */
private[versions] abstract class ExtendedVersionCompanion[V <: VersionBase[V, EV], EV[E] <: ExtendedVersionBase[
  V,
  E,
  EV
  ...
  def parseVersion[E](
      @deprecatedName('v, since = "1.3.0") version: String
  )(implicit ed: ExtensionDef[E], ep: ExtensionParser[E]): EV[E] = {
    version.split("-", 2) match {
      ...
      case Array(ver) =>
        ed.default match {
          ...
          case None =>
            throw new VersionFormatException(version, new UnsupportedOperationException("No default extension"))
        }
    }
  }
